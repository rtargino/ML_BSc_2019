...
def find_separator(negative, positive, w=[1, 1, 1]):
    ...
    """
    positives_x = list(map(lambda x: x[0], positive))
    ...
    points_p, points_n, line = plt.plot(positives_x, positives_y, 'bx',
    ...
    def update_frame(frame):
        ...
        if misclassified == []:
            ...
            w = list(map(lambda x, y:
                         x + y * target[2], w, list(target[:2]) + [1]))

            line.set_data([0, 10], [(-w[2]) / w[1],
                                    (- w[2] - w[0] * 10) / w[1]])

            return points_p, points_n, line

    line_ani = animation.FuncAnimation(
        fig1, update_frame, 10, interval=500, blit=True)
